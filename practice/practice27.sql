...
DELETE FROM 테이블명;
DML 데이터 삭제  조건으로 원하는 데이터만 삭제(where 특정 데이터 선택) commit / rollback 가능
TRUNCATE TABLE 테이블명;
DDL 데이터 삭제  전체 데이터 바로 삭제    삭제 시 바로 적용(auto commit)
;
...
--3. 
--다음 조건에 맞는 시퀀스를 생성하시오. -> 생성한 시퀀스는 id 컬럼에 사용
...
--4. 
--생성한 시퀀스의 값을 불러서 임의의 데이터를 저장해보시오.
...
select *
from T_MEMBER_POINT
where id = 5;           --시퀀스가 계속 증가하기 때문에 id값만으로도 구분 가능
