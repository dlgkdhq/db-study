...
where deptno IN (select dcode
           ...
           --IN (0001, 1003, 1006, 1007) 포함된 갯수가 많을 때 in으로 써줌
...
--학생들 중에 학번이 9513인 학생보다 키가 작은 학생들 조회
select *
from student;
...
select
    ...
    (select dname
        from dept2
        where dcode = deptno) -- 행의 갯수에 맞게 맞춰줘야 함. 하나면 하나 
--        where dcode > deptno)
from emp2;
...
select 
    emp_number, emp_name    -- 별칭 정해준 컬럼명으로 사용해야함
from (
    select
        e.empno emp_number,
        e.ename emp_name,
...
emp테이블, dept 테이블 조회. 부서번호와 부서별 최대 급여, 부서명 조회
10 5000 ACCOUNTING
