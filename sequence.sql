...
INCREMENT BY 1 -- 증가할 값. 1씩 증가
START WITH 1 -- 시작 값
MINVALUE 1 -- 최소값
MAXVALUE 10 -- 최대값
CYCLE | NOCYCLE -- 순환 여부     default : NOCYCLE
CACHE 20 | NOCACHE -- 숫자 메모리에 미리 저장해둘 갯수. 20개를 미리 저장하겠다
;
...
시퀀스 호출(사용)
시퀀스이름.nextval 다음 값 호출(사용)
시퀀스이름.currval 현재 값 확인
;
...
--INSERT INTO seq_test VALUES(3, '이름3'); --3 대신 삽입
...
INSERT INTO seq_test VALUES( (select COUNT(*)+1 from seq_test), '이름6'); --안됨.. 왜? 3이 탈퇴하고 count 하면 5인데 이미 5가 있는 상태임
...
INSERT INTO seq_test VALUES( (select MAX(no)+1 from seq_test), '이름1'); --안됨.. 기존 데이터가 없을 시, MAX(no) -> NULL 인식
...
select seq_seq_test_pk.nextval from dual; --다음 차례 seq 진행값 호출, 한번 지나간건 사용 불가
...
DROP SEQUENCE seq_seq_test_pk;  --지우고 다시 만들기
...
-- -> 다음에 불렀을 때 1이 될 수 있게 값을 조정 
ALTER SEQUENCE seq_seq_test_pk INCREMENT BY -8; --현재값 확인해서 값 조정
