...
select * from student; -> profno로 -> professor와 연결 - 외래키
select * from emp; -> deptno로 -> dept와 연결

CREATE TABLE tt03
(
    no NUMBER(3) PRIMARY KEY, --NOT NULL, UNIQUE 둘다 걸려있음
...
/*********************************************/
외래키 FK
- 다른 테이블에 있는 키값을 참조하는 컬럼
- 참조하는 테이블(부모테이블)에 존재하는 값만 사용 가능(+null 가능)
- 외래키가 설정된 부모테이블에서 자신을 참조하는 자식테이블의 데이터가 존재하면 삭제 불가
        
외래키 삭제 조건
ON DELETE CASCADE;  --부모 데이터 삭제 시, 그 값을 참조하는 자식 데이터도 같이 삭제
ON DELETE SET NULL; --부모 데이터 삭제 시, 그 값을 참조하는 자식 데이터를 NULL 값으로 변경
...
INSERT INTO T_MEMBER VALUES (5, '이름5', 8); --FK에 해당하는 T_CLUB에 id에 8이 존재하지 않아서 불가
