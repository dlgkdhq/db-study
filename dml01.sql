...
--전체 컬럼에 데이터를 저장 + 순서 맞춰서 저장하는 경우에는 컬럼명 작성 생략 가능
INSERT INTO 테이블명
VALUES (데이터 값...)
;
...
--select * from dept3  -- 바꾸려는 값들이 맞는지 확인하고 바꾸기
WHERE area = 'Seoul Branch Office';

UPDATE dept3
SET dname = 'Sales First Team';
--WHERE dcode = 1008; 조건 안주면 모든게 바뀌므로 신중하게 바꿔야함
...
commit;     확정  -- 커밋하면 롤백 불가
rollback;   되돌리기(취소) -- 원래 상태로 되돌림
